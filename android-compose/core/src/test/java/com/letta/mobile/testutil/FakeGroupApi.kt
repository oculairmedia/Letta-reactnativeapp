...
class FakeGroupApi : GroupApi(mockk(relaxed = true)) {
    ...
    override suspend fun createGroup(params: GroupCreateParams): Group {
        ...
        if (shouldFail) throw ApiException(500, "Server error")
        val group = Group(id = "group-${groups.size + 1}", managerType = "round_robin", agentIds = params.agentIds, description = params.description, projectId = params.projectId, sharedBlockIds = params.sharedBlockIds ?: emptyList(), hidden = params.hidden)
        groups.add(group)
        return group
    }
