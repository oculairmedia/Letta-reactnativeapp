...
class FakeProviderApi : ProviderApi(mockk(relaxed = true)) {
    ...
    override suspend fun createProvider(params: ProviderCreateParams): Provider {
        ...
        if (shouldFail) throw ApiException(500, "Server error")
        val provider = Provider(id = "provider-${providers.size + 1}", name = params.name, providerType = params.providerType, apiKey = params.apiKey, baseUrl = params.baseUrl, accessKey = params.accessKey, region = params.region)
