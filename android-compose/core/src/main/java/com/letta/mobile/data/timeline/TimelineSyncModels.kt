...
internal fun TimelineEvent.Confirmed.hasAlreadyIngestedStreamFrame(
...
internal fun latestSeqId(left: Int?, right: Int?): Int? = when {
    left == null -> right
    right == null -> left
    else -> maxOf(left, right)
}

internal fun hydratedMessageTypePriority(message: LettaMessage): Int = when (message) {
    is UserMessage -> 0
