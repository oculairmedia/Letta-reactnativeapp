...
object TextMessageRenderer : MessageContentRenderer {
    ...
    override fun Render(
        ...
        if (message.role == "user") {
            ...
            // arrive every ~1s. The committed prefix only re-parses on
            // boundary advances, so MarkdownText's heavier block layout
            // work runs at paragraph cadence instead of every paint tick.
            // Mid-block chunks only mutate the active tail subtree.
